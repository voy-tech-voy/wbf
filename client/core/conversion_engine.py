    ...
    def video_to_gif(self, file_path: str) -> bool:
        ...
        try:
            ...
            if gif_size_mode == 'max_size':
                ...
                self.status_updated.emit(f"Max Size mode: Optimizing for ≤{target_size_mb} MB")
                
                # Check if auto-resize is enabled
                auto_resize = self.params.get('gif_auto_resize', False)
                if auto_resize:
                    self.status_updated.emit("Auto-resize enabled: will try resolution scaling before low quality")
                ...
                if optimized_params.get('_target_exceeded'):
                    ...
                    self.status_updated.emit(f"Optimized settings found. Estimated size: {estimated_mb:.2f} MB")
    ...
    def _convert_video_to_gif_ffmpeg_only(self, file_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            if resolution_scale and resolution_scale < 1.0:
                ...
                self.status_updated.emit(f"Applying auto-resize: {resolution_scale*100:.0f}% of current size")
            ...
            dither_value = self.params.get('dither', 3)  # Default to 3 (medium quality)
            
            # Map dither value 0-5 to FFmpeg dither algorithms
            # 0 = none, 1-2 = bayer (low quality), 3-4 = sierra (medium-high), 5 = floyd_steinberg (best)
            dither_map = {
                0: 'none',
